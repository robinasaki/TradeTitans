...
public class APIDataAccessObject {
    ...
    private static void checkDateCondition(Date startDate, Date endDate) {
        LocalDate localDate = LocalDate.now();
        Date today = Date.from(localDate.atStartOfDay(ZoneId.systemDefault()).toInstant());
        if (!startDate.before(endDate)) {
            // exception case: endDate < startDate
            throw new RuntimeException("Start Date must be before the End Date.");
        } else if (endDate.after(today)) {
            // exception case: endDate > today
            throw new RuntimeException("End Date must be before today.");
        }
    }

    public HashMap<Date, Double> getHistoricalQuotes(String symbol, Date startDate, Date endDate) {
        checkDateCondition(startDate, endDate);
        ...
        return quotes; // TODO: date filtering
    }
